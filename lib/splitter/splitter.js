...
$.fn.splitter = function () {
    ...
    return this.each(function () {
        ...
        $movePanel.on('mousemove', function (event) {
            if (dragging) {
        ...
        $movePanel.on('mouseup touchend mouseleave', function () {
            if (dragging) {
                dragging = false;
                $blocker.remove();
                $movePanel.removeClass("active");
                // $handle.css( 'opacity', '0');
                // $body.removeClass('dragging');
                $sizeRecord.removeClass('dragging');
            }
        ...
        $blocker.on('mousemove touchmove', function (event) {
            if (dragging) {
                moveSplitter(event[props[type].moveProp] || event.originalEvent.touches[0][props[type].moveProp]);
            }
        });
        ...
        $handle.on('mousedown touchstart', function (e) {
            ...
            $sizeRecord.addClass('dragging');
            $movePanel.addClass("active");
        ...
        $handle.on('init', function (event, x) {
